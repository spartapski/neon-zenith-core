        ...
        <div className="grid gap-8 lg:grid-cols-12">
          <aside className="lg:col-span-4">
            <div className="glass p-8">
              <h3 className="text-xl font-bold text-white">{t("info.title")}</h3>
              <ul className="mt-6 space-y-5 text-sm text-white/80">
                <li className="flex items-start gap-3">
                  <MapPin className="mt-0.5 h-5 w-5 text-[color:var(--brand-violet)]" />
                  <div>
                    <div className="font-semibold text-white">{t("info.address.label")}</div>
                    <div className="text-white/70">{t("info.address.value")}</div>
                  </div>
                </li>
                <li className="flex items-start gap-3">
                  <Phone className="mt-0.5 h-5 w-5 text-[color:var(--brand-violet)]" />
                  <div>
                    <div className="font-semibold text-white">{t("info.phone.label")}</div>
                    <div className="text-white/70">{t("info.phone.value")}</div>
                  </div>
                </li>
                <li className="flex items-start gap-3">
                  <Mail className="mt-0.5 h-5 w-5 text-[color:var(--brand-violet)]" />
                  <div>
                    <div className="font-semibold text-white">{t("info.email.label")}</div>
                    <div className="text-white/70">{t("info.email.value")}</div>
                  </div>
                </li>
                <li className="flex items-start gap-3">
                  <Clock className="mt-0.5 h-5 w-5 text-[color:var(--brand-violet)]" />
                  <div>
                    <div className="font-semibold text-white">{t("info.hours.label")}</div>
                    <div className="text-white/70">{t("info.hours.value")}</div>
                  </div>
                </li>
              </ul>

              <div className="mt-8">
                <div className="mb-3 text-xs font-semibold uppercase tracking-wider text-white/60">{t("info.social")}</div>
          ...
          <div className="lg:col-span-8">
            ...
            >
              ...
              <label className="mt-5 block">
                <span className="mb-2 block text-xs font-semibold uppercase tracking-wider text-white/70">{t("form.message")}</span>
                <textarea
                  name="message"
                  rows={6}
                  required
              ...
              <div className="mt-8 flex flex-wrap items-center gap-4">
                ...
                {sent && (
                  <span className="inline-flex items-center gap-2 rounded-full border border-emerald-400/30 bg-emerald-400/10 px-4 py-2 text-sm text-emerald-300">
                    <Check className="h-4 w-4" /> {t("form.success")}
                  </span>
                )}
              </div>
            </form>
